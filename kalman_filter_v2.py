...
def main_kalman(last_face_touch):
    ...
    if mu < 0.2:
        #lightbulb(True)
        count_face_touch += 1
        #os.system("python3 face_touch_sound.py")
        file = open('make_sound.txt', 'w')
        file.seek(0)
...
def rude_awakening():
    #pygame.mixer.init()
    #pygame.mixer.music.load("/tmp/voice.wav")
    #pygame.mixer.music.play()
    #while pygame.mixer.music.get_busy() == True:
     #   continue
    #playsound("/tmp/voice.wav")
    #call(["aplay", "/tmp/voice.wav"])
    #mixer.init()
    #sound = mixer.Sound("/tmp/voice.wav")
    #while True:
     #   sound.play()
...
def lightbulb(flag):
    ...
    if flag:
        #rude_awakening()
        GPIO.output(RED,1)#str(msg.payload)[1])
        time.sleep(2)
        #GPIO.output(RED,0)
        #time.sleep(2)
    else:
